"""
Metadata Extraction and Analysis Module
Extracts comprehensive metadata from datasets for efficient comparison and change tracking
"""
...
class ColumnMetadata:
    ...
    value_hash: str  # Hash of sorted unique values
    stats_hash: str  # Hash of statistical summary
...
class DatasetMetadata:
    ...
    structure_hash: str  # Schema + column order
    content_hash: str    # Sample of data content
    stats_hash: str      # Statistical summary
...
class MetadataExtractor:
    """Extract comprehensive metadata from datasets"""
    ...
    def extract_metadata(self, df: pd.DataFrame, name: str, source_type: str, 
        ...
        """Extract complete metadata from a DataFrame"""
        ...
        for col in df.columns:
            columns_metadata[col] = self._extract_column_metadata(df[col], col)
        
        # Calculate fingerprint hashes
        structure_hash = self._calculate_structure_hash(df)
        content_hash = self._calculate_content_hash(df)
        stats_hash = self._calculate_stats_hash(df)
    ...
    def _extract_column_metadata(self, series: pd.Series, col_name: str) -> ColumnMetadata:
